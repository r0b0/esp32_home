...
io.on("connection", (socket) => {
  ...
  //
  // Server starts emitting periodic `vInfo` events every 2 seconds for each
  // vehicle approaching the requested stop(s).
  // -------------------------------------------------------------------------
  socket.on("tabStart", (zastavka, nastupiste, region) => {
    ...
    platforms.forEach((platform) => {
      ...
      console.log(`  -> starting vInfo stream for key=${key}`);

      // Emit an immediate first update, then continue every 2 s
      socket.emit("vInfo", randomVehicle());
      ...
      const interval = setInterval(() => {
        ...
        console.log(
          `[vInfo]    socket=${socket.id} key=${key} vehicle=${vehicle.issi}`
        );
      }, 2000);
